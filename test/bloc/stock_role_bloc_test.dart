...
void main() {
  test('StockRoleBloc initializes with correct initial state', () {
    ...
    final wallet = Wallet();
    final stockRole = StockRole(catalog: stockCatalog, stock: stock, wallet: wallet);
    final stockRoleBloc = StockRoleBloc(stockRole: stockRole);
    expect(stockRoleBloc.state.runtimeType, InitialStockRoleState);
    stockRoleBloc.close();
  });

  blocTest<StockRoleBloc,StockRoleState>('StockRoleBloc emits LoadingIngredientsStockRoleState when LoadIngredientsStockRoleEvent is added',
  build: () {
      final stockCatalog = IngredientsCatalog();
      final stock = IngredientsStock();
      final wallet = Wallet();
